...
function getMacroOptionsJson (componentName) {
  try {
    ...
    // if (JSON.parse(fs.readFileSync(optionsFilePath, 'utf8'))) {
    ...
    // try {
    //   let optionsFilePath = path.join(paths.lbsfrontendcomponents, componentName, 'macro-options.json')
    //   console.log(optionsFilePath)
    //   return JSON.parse(fs.readFileSync(optionsFilePath, 'utf8'))
    // } catch (err) {
      console.error(err)
...
function getAdditionalComponentOptions (options) {
  ...
  let namesWithoutDuplicates = names.filter((optionName, index) => {
