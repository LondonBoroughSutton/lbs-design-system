...
module.exports = metalsmith(__dirname) // __dirname defined by node.js: name of current working
  ...
  .use(assets({
    source: path.join('../', paths.lbsfrontend, '/LBS/assets/'),
    destination: 'assets'
  }))
  ...
  // build LBS Frontend JavaScript
  .use(rollup({
    input: path.normalize('src/javascripts/lbs-frontend.js'),
    output: {
      legacy: true,
      format: 'iife',
      file: path.normalize('javascripts/lbs-frontend.js')
    },
    plugins: [
      // TODO: Since we're not exporting ES6 Modules to NPM we need to import these as commonjs
      resolve(),
